...
{
    public static final Logger LOG = Log.getLogger(HttpReceiverOverHTTP2.class);
    ...
    public void onHeaders(Stream stream, HeadersFrame frame)
    {
        ...
        LOG.debug("exchange == null: "+ (exchange == null));
        if (exchange == null)
            return;
        ...
        LOG.debug("metaData.getHttpVersion(): "+ metaData.getHttpVersion());
        LOG.debug("metaData.getStatus(): " + metaData.getStatus());
        LOG.debug("metaData.getReason(): " + metaData.getReason());
        ...
        if (responseBegin(exchange))
        {
            ...
            {
                LOG.debug("responseHeader:" + header.getName());
                if (!responseHeader(exchange, header))
                    return;
                LOG.debug("responseHeader with a success");
            }

            if (responseHeaders(exchange))
            {
                LOG.debug("responseHeaders with a success");
                ...
                if (frame.isEndStream() || informational) {
                    ...
                    LOG.debug("responseSuccess with a success");
                }
            }
        }
    }
